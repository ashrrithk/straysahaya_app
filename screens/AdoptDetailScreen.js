...
export default function AdoptDetailScreen() {
  ...
  const handleEnquiry = async () => {
    try {
      ...
      if (result[0]?.email?.length > 0 && result[0].email[0]) {
        ...
        navigation.navigate("AdoptRequestForm", {
          name: item?.name,
          helpCenter: item?.help[0]?.name,
        });
      } else {
  ...
  /> */}
          ...
          <View>
            <Text className="text-3xl font-semibold ml-5 mt-5">
              {item?.name}
            </Text>
            ...
            <View className="flex-row ml-5 mt-2 items-center">
              <Icons.MapPin
                ...
                stroke="black"
              />
              <Text className="text-md font-medium ml-2 mt-2">
                {item?.help[0]?.name}
              </Text>
            </View>
            {item?.about?.description ? (
              <View className="flex-row items-center mt-5 ml-5">
                <Ionicons
                  ...
                  color="black"
                />
                <Text className="text-md font-medium leading-5 ml-2">
                  {item?.about?.description}
                </Text>
              </View>
            ) : null}
          ...
          {/* About Section */}
          <View
            className="flex-col m-3 mt-5 "
            style={{ backgroundColor: "#F5F9FA", borderRadius: 20 }}
          >
            <Text className="m-3 text-black font-semibold text-lg">About</Text>
            <View className="flex-row flex-wrap">
              <View
                className="flex-row rounded-full px-4 py-2 m-2"
                style={{
          ...
          <View className="items-center mb-5 mt-5">
            <TouchableOpacity
              className="h-14 w-11/12 bg-text rounded-lg items-center justify-center"
